...
- **`page.screenshot({clip})` silently captures the wrong thing once the planner is scrolled.** Puppeteer resolves a clip against *document* coordinates, but the planner scrolls an inner container, so `getBoundingClientRect()` (viewport) and the clip disagree by the scroll offset — a capture aimed at a factory near the bottom returned a completely different card, convincingly and with no error. `elementHandle.screenshot()` has the same flaw. Pass `captureBeyondViewport: false` and a viewport-relative clip. Also note `.export-calculator` and `.factory-card` exist once per factory, so scope every query to the card you mean; `.factory-card` matches sub-cards too.
