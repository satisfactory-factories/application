...
- **There are THREE stacked first-run modals, and suppressing one reveals the next.** Set all of them or your clicks silently land on an overlay: `dismissed-introduction: 'true'` (welcome modal), `seenV51Splash: 'true'` (the "What's new" slideshow, which only appears once the intro is dismissed), `buildingGroupTutorialOpened: 'true'` (fires on first Building Groups open). Symptom is a click that no-ops; confirm with `document.elementFromPoint()` on the target's centre.
