...
- Parts & Recipes: recipe cards now lay out in a responsive grid (three across on desktop, two on medium screens, one on mobile) instead of stacking full-width, and "Produced by" became its own accordion (open by default) alongside "Alternate Recipes" and "Used in". The accordions got a greyer background, light border and shadow so they stand out from the part panel, recipe cards inside are darker for contrast, and part entries in the list are now slightly lighter than the page background.
