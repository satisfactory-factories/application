...
- **Effective buildings** = Σ `buildingCount × overclock/100 × sloopBoost × extractionBoost`. Overclock is a *linear* production multiplier; so are the somersloop and extraction boosts, making "effective" mean *output*-effective. "Remaining" = item's required buildings − effective (negative = over-producing).
- **Extraction** (`building-groups/extraction.ts`): a mine's miner mark and node purity are expressed as an output multiplier *relative to the recipe's reference extractor* (Miner Mk.1 on a normal node, 60/min), so a Mk.3 group on pure nodes counts as `(240/60) × 2 = 8` effective buildings and the solver needs no re-derivation. Purity changes yield, never power; power comes from the group's own extractor. Because the unit is reference-extractor equivalents, extraction items report **Effective Output** (`Σ effective × referenceRate`) rather than effective buildings — "88 buildings short" would mean 88 Mk.1-equivalents. A resource well is one group: `buildingCount` is pressurizers and `satellites` counts the micro-nodes by purity, which draw no power of their own.
