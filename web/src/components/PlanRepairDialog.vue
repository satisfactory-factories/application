  ...
  >
    ...
    <div v-for="group in groupedRepairs" :key="group.factoryName" class="mb-4">
      ...
      <ul class="repair-list">
        <li v-for="(repair, index) in group.repairs" :key="index" class="mb-1">
...
<script setup lang="ts">
  ...
  // Dismissed however it was closed — the corner button, Esc, or the scrim — so a repair list
  // that has been seen is never raised a second time.
  watch(isOpen, open => {
    if (!open) appStore.dismissPlanRepairs()
  })
