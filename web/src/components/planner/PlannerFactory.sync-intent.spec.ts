...
describe('Component: PlannerFactory (header sync intent)', () => {
  let factory: Factory

  const mountSubject = () =>
    mount(PlannerFactory, {
      props: { factory, totalFactories: 1 },
      global: {
        plugins: [vuetify],
        stubs,
        provide: {
  ...
  it('writes a typed name onto the factory once the field is left', async () => {
