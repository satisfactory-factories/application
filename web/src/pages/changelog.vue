  ...
  <v-container max-width="1200">
    <v-row>
      <v-col ref="content" cols="12">
        ...
        <p>A switch <b>in the account panel</b> — on the cloud account tile, not under Options — puts the planner into offline mode, which means exactly no contact with the server: no connection, no requests, no retries. Keep planning; everything is kept on this device.</p>
        <ul class="ml-6 mt-2">
        ...
        <ul class="ml-6 mt-2">
          ...
          <li><b>The factory's Checklist chip counts them</b> — <code>Checklist: 12/14 · 2 to reconfirm</code> — instead of saying <code>(desynced)</code>, so a collapsed factory card tells you whether it is one stale row or nine.</li>
          <li><b>A desynced checklist is now a factory status of its own</b>, so it wears an amber <b>Checklist desync</b> chip everywhere the other statuses appear — the card header, the sidebar entry, the Factories Summary and the plan- and group-level tallies — carrying the icons of the items that moved. Previously the only clue in the sidebar was a counter quietly changing colour, which said a factory was amber without saying why.</li>
          <li><b>The Checklist panel is three tables side by side</b> — Products (with Power beneath it), Imports and Exports — instead of one list stacked four groups deep. Imports and Exports are listed by item, with a tick per source or destination factory hanging off it, so a factory buying three parts from the same neighbour names that neighbour once per part rather than repeating the item name for every factory it deals with.</li>
        ...
        <ul class="ml-6 mt-2">
          ...
          <li><b>An over-committed mine can be given imports again</b> (#541). A mine shipping out more than it extracts matched the "nothing here could ever import anything" shortcut — extraction takes no ingredients, so its Add Import button was disabled — even with another mine in the plan able to cover the difference. Together with #540 the two halves are coherent: the mine goes red and the fix for it is available in the same place.</li>
