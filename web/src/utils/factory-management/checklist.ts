// Checklist mode: lets the player tick off products, imports and exports as they build them in
// game. Products and inputs carry their own `completed` flag directly (they are user-authored,
// persisted arrays). Exports have no equivalent home: a factory's exports are derived from other
// factories' inputs and rebuilt by the dependency engine, so storing state directly on a request
// risks it being silently dropped. Ticks for exports are therefore kept in factory.checklistExports,
// keyed by the (destination factory, part) pair the export chip actually represents.
...
// Total checklist items this factory currently has: one per product, per import, and per
// (destination, part) export pair. Recomputed rather than stored, so it always matches the plan.
export const countChecklistTotal = (factory: Factory): number =>
  factory.products.length + factory.inputs.length + getRequestsForFactory(factory).length
...
export const countChecklistCompleted = (factory: Factory): number => {
  ...
  const exports = getRequestsForFactory(factory)
    ...
    .length
  return products + inputs + exports
}
